export const INFO = `	대표: 송지은 | 개인정보 보호 책임자: 송지은
	광고 및 제휴 문의: findev@omoapp.co.kr
	주소: 동대문구 경희대로 26 삼의원창업센터 2층 216호 | 대표번호: 010-4818-2081
	사업자등록번호: 148-86-02774
	통신판매업신고번호: 2023-서울동대문-1310`;

export const INQUIRY = `	@카카오톡채널 omo
	운영시간 : 9:00 - 18:00
	점심시간 : 12:00 - 13:00`;

export const RESPONSIBILITY = `	(주) 핀데브는 통신판매중개자로서 거래당사자가 아니며, 호스트가 등록한 상품정보 및 거래에 대해 (주)핀데브는 일체의 책임을 지지 않습니다.
	omo 기획전 상품에 대한 문의, 환불은 (주)핀데브에서 처리하며 이에 대한 책임은 (주) 핀데브에 있습니다.`;

export const COPYRIGHT = `ⓒ 2026 FINDEV. CO. LTD ALL RIGHTS RESERVED`;
