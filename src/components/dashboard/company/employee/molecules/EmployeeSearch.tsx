...
export default function EmployeeSearch({ onSearch, currentDateRange }: Props) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await getData(
        `v1/manager/member/list?page=1&search=${currentSearchValue.term}&filter=${currentSearchValue.field}&startDate=${currentDateRange.startDate?.toISOString().split('T')[0]}&endDate=${currentDateRange.endDate?.toISOString().split('T')[0]}`,
        true
      );
